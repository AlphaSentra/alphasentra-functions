...
from Functions.helpers import get_ticker_and_name_by_etoro_symbol
...
def generate_portfolio_holdings_analysis(risk_contrib, sector_industry_df, price_data, portfolio_df):
    ...
    # Backfill missing name/sector for eToro symbols via MongoDB
    holdings['name'] = holdings.get('name', '').fillna('')
    holdings['sector'] = holdings.get('sector', '').fillna('')

    for ticker in list(holdings.index):
        row_name = str(holdings.at[ticker, 'name']) if ticker in holdings.index else ""
        if not row_name or row_name.lower() == 'nan':
            standard_ticker, name = get_ticker_and_name_by_etoro_symbol(str(ticker))
            if name:
                holdings.at[ticker, 'name'] = name
            if standard_ticker and standard_ticker != str(ticker):
                holdings.at[ticker, 'ticker_mongo'] = standard_ticker
            else:
                holdings.at[ticker, 'ticker_mongo'] = str(ticker)
        else:
            holdings.at[ticker, 'ticker_mongo'] = str(ticker)
    ...
    def render_view_table(view_name, config):
        ...
        for ticker, row in holdings.iterrows():
            ...
            sector = row['sector']
            if pd.isna(sector) or sector == "Unknown" or str(sector).lower() == 'nan':
                sector = "Others"
